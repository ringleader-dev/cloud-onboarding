...
| `flow_log_reader_ip_rules` | `[]` | Public IPv4 addresses or CIDR ranges allowed to read the flow log blobs. The storage account's firewall denies every other network, except Azure trusted services, which is the exception Network Watcher writes through. Empty admits no reader at all. Add an address here rather than on the account, because a rule added by hand is removed on the next apply. See `azure/README.md`. |
| `create_flow_log_private_endpoint` | `false` | Reach the flow log storage account over a private endpoint in this landing pad's VNet, with the private DNS zone and its link. **Off by default**, because a private endpoint bills hourly plus per GB. Turn it on when a reader runs inside the storage account's region, which `flow_log_reader_ip_rules` cannot admit, or when a compliance scan asks for private endpoints by name. Needs `create_flow_logs`, and the plan refuses it without. Turning it back off leaves the endpoint in place: see `azure/README.md`. |
| `flow_log_private_endpoint_subnet_prefix` | `null` → the `/24` above the gateway range | Range for that endpoint's subnet, which is carved whenever `create_flow_logs` is set and stays empty until an endpoint lands in it. Derived from the VNet's own `/16`, so a region that takes the derived ranges needs nothing here. |
