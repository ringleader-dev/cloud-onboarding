...
| `enable_egress_control` | **`true`** | Let Ringleader manage the NSGs an egress policy compiles to, **and** the route tables and subnets that steer traffic at the DNS / HTTPS proxy. Adds twenty-three actions, still scoped to this resource group. Restricts nothing until you declare a policy. |
