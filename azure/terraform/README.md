...
| `create_gateway_subnet` | **`true`** | Reserve the subnet the DNS / HTTPS proxy VM runs in. Hand `gateway_subnet_id` back as `EgressGateway.spec.subnet`; no gateway VM is built until you do. Azure does not bill for the subnet, but the VM Ringleader builds in it carries its own standalone public IP, billed separately. |
