...
output "role_extras_granted" {
  ...
  value = concat(
    ...
    var.enable_egress_control ? ["Microsoft.Network/networkSecurityGroups (+ securityRules) and routeTables (+ routes) read/write/delete and join/action, plus virtualNetworks/subnets/write -- egress control"] : [],
  )
}
