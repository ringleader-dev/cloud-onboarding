...
variable "enable_egress_control" {
  ...
  description = <<-EOT
    ...
    It adds fifteen actions to the custom role, still scoped to this one resource group:
      ...
      - route-table read/write/delete (with its routes and join/action) plus subnet write,
        which is how a workstation's traffic is steered to the DNS / HTTPS proxy when a policy
        names hostnames. An Azure route table attaches per subnet, so per-policy steering
        needs a subnet per policy.
