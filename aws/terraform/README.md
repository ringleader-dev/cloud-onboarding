...
| `ssh_source_ranges` | `[]` | inbound-SSH CIDRs. Empty creates **no rule of your own**. Ringleader attaches its own security group admitting TCP 22 and 2222 from any address to the workstations it creates. Set this to reach other instances, or to keep your engineers' CIDRs working when Ringleader cannot create that group |
...
| `enable_egress_control` | **`true`** | let Ringleader manage the security groups an egress policy compiles to, **and** the subnets and route tables that steer traffic at the proxy. Grants seven security-group actions, nine route-table and subnet writes, `ec2:ModifyNetworkInterfaceAttribute`, the five Elastic IP actions that reserve the gateway's address, and two reads. Restricts nothing until you declare a policy. Also lets Ringleader create the security group that admits SSH to its workstations |
| `egress_vpc_ids` | `[]` | confine those permissions to these VPCs. Empty uses the VPC this module created; empty **and** no created network means scoping by `allowed_regions` only, or none if that is empty too. Check the `egress_scope` output |
| `enable_artifact_storage` | **`true`** | let Ringleader hold artifact payloads — sealed agent-session transcripts, workflow file outputs, files a box publishes — in an S3 bucket in **this account** rather than in Ringleader's own. Bounded by **ARN pattern** to buckets named `ringleader-*`, not by region: S3 is global, and a region bound would break a bucket you placed elsewhere for residency. Never `PutBucketPolicy` and never an ACL write. Writes nothing until a Ringleader namespace declares a `Storage` naming a bucket, and the S3 backend itself ships after the GCS one — the grant arrives early so that does not cost a second apply |
| `artifact_storage_bucket` | `""` (managed) | name a bucket **you** created and the grant narrows to that one ARN, with no `CreateBucket`, `DeleteBucket` or lifecycle write — its region, lifecycle rules and default encryption stay yours. Empty takes the managed width, where Ringleader creates and converges its own `ringleader-*` buckets |
| `create_gateway_subnet` | **`true`** | reserve an empty **public** subnet, plus its route table, for the DNS / HTTPS proxy VM. You hand its id back as `spec.subnet` on the `Edge`, and Ringleader builds no gateway until you do. Public and in the workstations AZ are both cost decisions — see `aws/README.md` |
